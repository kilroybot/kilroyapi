...
SHELL ["conda", "run", "--no-capture-output", "-n", "kilroyapi", "/bin/bash", "-c"]

# add poetry files
COPY ./kilroyapi/pyproject.toml ./kilroyapi/poetry.lock /tmp/kilroyapi/
WORKDIR /tmp/kilroyapi

ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "kilroyapi"]
...
COPY ./kilroyapi/src/ /tmp/kilroyapi/src/
COPY ./kilroyapi/tests/ /tmp/kilroyapi/tests/
COPY ./kilroyapi/LICENSE ./kilroyapi/README.md /tmp/kilroyapi/

# build wheel by poetry and install by pip (to force non-editable mode)
RUN poetry build -f wheel && \
    python -m pip install --no-deps --no-index --no-cache-dir --find-links=dist kilroyapi
